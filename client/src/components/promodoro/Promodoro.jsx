  ...
  switch (action.type) {
    ...
    case "ACTION_TICK":
      if (state.seconds > 0) {
        ...
        if (state.mode === "work") {
          ...
          newModes = newCycles % state.settings.cyclesBeforeLongBreak === 0 ? "longBreak" : "shortBreak";
        } else {
          newModes = "work";
        }
    ...
    case "ACTION_SKIP":
      ...
      if (state.mode === "work") {
        ...
        newMode = newCycles % state.settings.cyclesBeforeLongBreak === 0 ? "longBreak" : "shortBreak";
      } else {
        newMode = "work";
      }
  ...
  return (
      <div className={styles.container}>
        <div className={styles.modeBadgeContainer}>
          <div
              className={`${styles.modeBadge} ${state.mode === "work" ? styles.modeBadgeFocused : ""}`}
              onClick={() => dispatch({ type: "ACTION_RESET" })}
          >
            Travail
          </div>
          <div
              className={`${styles.modeBadge} ${state.mode === "shortBreak" ? styles.modeBadgeFocused : ""}`}
              onClick={() => dispatch({ type: "ACTION_RESET" })}
          >
            Pause Courte
          </div>
          <div
              className={`${styles.modeBadge} ${state.mode === "longBreak" ? styles.modeBadgeFocused : ""}`}
              onClick={() => dispatch({ type: "ACTION_RESET" })}
          >
            Pause Longue
          </div>
        ...
        <div className={styles.buttonContainer}>
          {!state.isRunning ? (
              <button className={`${styles.button} ${styles.startButton}`} onClick={startTimer}>
                Démarrer
              </button>
          ) : (
          ...
          <button className={`${styles.button} ${styles.skipButton}`} onClick={skipTimer}>
            Passer
          </button>
        </div>
      </div>
